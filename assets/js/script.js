...
function fixDiv() {
  //alert('11');
    ...
    if ($(window).scrollTop() > $div.data("top")) { 
       //alert('222');
        $div.css({'position': 'fixed', 'top': '0', 'width': '100%','background':'rgba(237,237,237,0.9)','z-index':'999'}); 
        
        //$('#cssmenu-fix').addClass('container');

    }
    else {
        $div.css({'position': 'static', 'top': 'auto', 'width': '100%','background':'none'});
            $('#cssmenu-fix').addClass('container');
...
$("#cssmenu-fix").data("top", $("#cssmenu-fix").offset().top); // set original position on load
